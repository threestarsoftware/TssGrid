...
- **CSV出力**: `toCSV(opts)` / `downloadCSV(filename, opts)`。`opts`: `headers`(既定true) / `formatted`(既定false＝素の値で数値はそのまま, trueで表示形 `¥1,234`) / `skipEmpty`(既定true＝空行除外) / `eol`(既定 `'\r\n'`) / `sanitizeFormula`(既定true)。`"`/カンマ/改行を含むセルは自動でクォート。**CSV数式インジェクション対策も既定ON**＝`=`/`@`/`+`/`-` 始まりの非数値セルを `'` で無害化（Excel/Sheetsで式として実行されない。`-100` 等の数値は保全）。生値が要るなら `toCSV({sanitizeFormula:false})`。**CSV入力（読込）は非内蔵**＝Excelからの貼り付けで取り込めるため（パースが必要なら PapaParse 等を）。
...
grid.sortBy(2, 'desc');
grid.sortByCols([                  // ★ 多段（複数列）: 先頭=第1キー。同値のときだけ次段で決める
  { col: '部署', dir: 'asc'  },
  { col: '売上', dir: 'desc' },
]);
grid.sortRows((a, b) => a.qty - b.qty);   // 任意比較（行はオブジェクト/配列）
```
- `sortBy`/`sortByCols`/`sortRows` は**データ実体を並べ替え**ます。整列・セル readOnly・checkbox・隠しフィールド（`_src`）は**行ごと追従**します。**Undo には積みません**（書式系と同じ割り切り）。
- `sortByCols` の各段は `sortBy` と**同じ比較規則**（数値/文字列自動判定）。安定ソートなので、全段が同値の行は**元の並びを保ちます**。**多段ソートのヘッダUI（▲▼アイコン＋優先度番号・本体クリックで列選択と両立）**は `onHeaderRender` を使った → 動く例: [`examples/multisort.html`](https://tssgrid.threestarsoftware.co.jp/examples/multisort.html)。
...
`rowDelete: true` で**右端に専用の「削除列」**が出て、各行の **🗑 を1クリックで当該行を削除**できます（右クリック→「行を削除」と同一経路）。削除は**Undo/Redo 対応**（`Ctrl+Z` で復活）なので確認ダイアログは出しません。非技術者にも分かる削除アフォーダンスとして、右クリックに気づかなくても消せます（→ 動く例: [`examples/row-delete.html`](https://tssgrid.threestarsoftware.co.jp/examples/row-delete.html)）。

```js
new TssGrid(el, {
  rowDelete: true,           // 右端に🗑削除列（既定 false）
  rowDeleteWidth: 36,        // 削除列の幅(px)・任意（既定 36）
  rowReorder: 'header',      // 併用可: 左=⠿で移動 / 右=🗑で削除（役割が左右で分かれる）
});
```
- 削除列は**非データのシステム列**です。`getData`/`getRows`/`toCSV`・列インデックス・列選択・リサイズ・並べ替えの**対象外**で、データ列とは混ざりません。データ列の右隣に並ぶ末尾列で、横幅がはみ出す表では横スクロールで表示されます（表がはみ出す時に🗑を貼り付けると、選択枠やデータ列と重なるため貼り付けません）。
- **最後の1行**（0行にはできない）で🗑を押すと、**削除ではなく「中身クリア（行は残す）」**になります＝🗑は無効にならず、行の内容だけ空に（`readOnly` セルは保持・1 undo で復活。ツールチップも「内容をクリア」に変わります）。「消してから空行を足す」のと同じ結果を、0行に一瞬も落とさず1手で。
- **`allowDeleteRows: false` なら削除列そのものを出しません**。`minRows` を **2 以上**に設定した場合は下限で🗑を**淡色の無効表示**にします（そのロック行は削除もクリアもしません）。
- 込み入った削除（一括・条件付きなど）は、削除列に頼らず**チェックボックス列＋「選択行を削除」ボタン**で組むのが素直です（🗑は「1行1クリック」の単機能に留めています）。

### ブラウザ自動入力の抑止（`suppressAutofill`）

グリッドは編集用に本物の `<input>` を重ねる方式なので、稀に **Chrome の自動入力（「ポイントカードを管理…」等）** がセル編集時に被さることがあります。既定で `name` ＋ `autocomplete="off"` ＋各パスワードマネージャの無視属性を付けて抑止していますが、**Chrome はポイントカード/住所/支払い系で `autocomplete="off"` を無視する**ことがあり、特に**周囲に自前のラベル付き `<input>`（フォーム塊）を並べた画面**（モーダルのヘッダ入力欄など）だと Chrome がそれをフォームと認識して誘発します。

その場合は `suppressAutofill: true` を渡すと、共有エディタの `autocomplete` を **`new-password`** に切り替えて確実に止めます（Chrome が `off` を無視する系にも効く唯一の実用手）。

```js
new TssGrid(el, { suppressAutofill: true });   // 既定 false
```
...
- **自作エディタの `inline` は既定 false**（宣言しない限り挙動は一切変わりません）。共有 input に乗るには実装側の合意が要るためで、ここは opt-in が正しい既定です。
- **同梱プラグインの `inline` は既定 true**：`TssAutocomplete`（打ちながら候補を絞る）／`TssCalendar`・`TssDatetime`・`TssTime`（**日付/時刻をそのまま直打ち**できる。カレンダーは打った内容に追従してその日へジャンプ）。**何も書かずにコピペした人が、看板どおりの「打ち始めれば書ける」を得ます**。
  - 日付系は **「キー派は打つ／マウス派はクリック」** の割り切り＝**矢印キーは奪いません**（文字カーソル/セル移動のまま）。伝票入力のように「`6/20` と打つ方が速い」場面向け。
  - `inline: false` で旧型（ポップアップにフォーカスし ←→↑↓ で日移動）に戻せます。ただし **セルにいきなり打ち始めて上書き**（表計算で一番よく使う操作）ができなくなる＝クリック/F2 で開いてから打つことになります。矢印での日送りが要る場面だけ選んでください。
...
```
- セル右端に **📅** が出て**シングルクリックで開閉**（再クリックで閉じる＝トグル）、開いている間も**フィルハンドルで下へコピー可**（ドロップダウンと同じ操作感）。**年・月はプルダウン**で直接ジャンプ。祝日は赤＋**名称ツールチップ**、土日は色分け。保存値は ISO のまま。
- **打つ人はそのまま打てます**（既定 `inline`）: セルに `6/20` と打ち始めればそのまま編集開始（**1文字目も落ちない・IME 直打ちも効く**）、カレンダーは打った日へ追従。確定は Enter か日をクリック。**矢印はセル移動のまま**（カレンダーは奪いません）。
  - 矢印/PageUp・Down でカレンダー内を移動する旧型が要るなら `inline: false`（→ [`editor.inline`](#editorinline-打鍵ime-直打ちを殺さないエディタサジェスト向け)）。**セルに直接打ち始める操作は失われます**。
...
| `minRows` / `maxRows` | `number` | — | 行数の下限/上限（min は初期パディング、削除の床。max は天井＝挿入・`minSpareRows` の空行・フィルハンドルの自動追加すべてを打ち止める） |
| `minCols` / `maxCols` | `number` | — | 列数の下限/上限（max は挿入の天井） |
...
| **列/行ヘッダを Ctrl+クリック** | 列・行を**飛び飛びに複数選択**（トグル。Ctrl+Shift で範囲を追加。`disjointSelect:false` で無効化）。選択は `grid.getSelectedColumns()` / `grid.getSelectedRows()` で取得（昇順・飛び飛び対応）。**飛び飛びの列/行は Ctrl+C コピー / Ctrl+X カットOK**（選んだ列×全行／選んだ行×全列＝Excelに貼れる長方形。カットは貼付先で元の列/行がクリア）。**Delete で内容クリア・右クリック「N 列/行を削除」で構造削除**もまとめて効く |
